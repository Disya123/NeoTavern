...
/**
 * Debug-only M0-D1b moving-sample probe. Not in the launcher. Start with:
 *
 * `adb shell am start -n com.neotavern.mobile/.M0D1bActivity`
 *
 * Extras: `com.neotavern.mobile.M0_D1B_FRAMES` (default 1000) and
 * `com.neotavern.mobile.M0_D1B_CAPTURE_FRAME` (default 120; `-1` disables
 * RenderDoc). Logs one `m0-d1b` line under tag `NeoTavern`.
 * Does not change production WebView [MainActivity].
 */
class M0D1bActivity : Activity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val view = TextView(this)
        view.text = "M0-D1b probe running…"
        view.textSize = 16f
        view.setPadding(48, 48, 48, 48)
        setContentView(view)
        logRefreshTelemetry()

        val frames = M0D1bLaunch.parseFrames(intent.getStringExtra(M0D1bLaunch.EXTRA_FRAMES))
        val captureFrame =
            M0D1bLaunch.parseCaptureFrame(intent.getStringExtra(M0D1bLaunch.EXTRA_CAPTURE_FRAME))
        thread(name = "m0-d1b") {
            val line =
                try {
                    M0D1bProbe.runDynamic(frames, captureFrame)
                } catch (err: Throwable) {
                    "m0-d1b gpu_ran=false ran_on_android=true capture=false verdict=BLOCKED reason=load_failed:${err.javaClass.simpleName}"
                }
            Log.i(TAG, line)
    ...
    private fun logRefreshTelemetry() {
        ...
        Log.i(
            ...
            ).replace("m0-d1a-refresh", "m0-d1b-refresh"),
        )
    }
