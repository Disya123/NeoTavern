---
editUrl: https://github.com/Disya123/NeoTavern/edit/main/docs/desktop/neocompositor-desktop-host.md
---
...
Winit-клавиатура подключена (бинарь): тап по композеру (`data-part="composer"`
внизу чат-области) или по полю поиска Character Manager ставит bin-local
`focus` (`TextFocus::{Composer, CharacterSearch}`), затем символы и Backspace
идут в `session.set_composer_text` / `set_character_search` (вью-модель уже
владела этими строками — не хватало только событий). Харнесс `--type "<text>"`
воспроизводит ввод после тапов, установивших фокус:

- композер: `--pointer 700,720 --type "hello"` → `typed … -> composer="hello"`;
...
Остальные builtin-действия (edit/checkpoint/rollback) теперь рисуются в
инлайн-ряде для визуального паритета с React, но copy и delete — единственные
подключённые к поведению. Edit / snapshots / regenerate остаются waiver.
