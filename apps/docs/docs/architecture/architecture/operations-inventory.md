...
| `chats.messages.variants.activate` | transactional | `POST /api/v2/chats/:id/messages/:mid/variants/:variantId/activate` (browser mode; legacy activates by variant id too, optional `expectedRevision` CAS is legacy-only); kernel mode records the replaced text as an immutable revision |
