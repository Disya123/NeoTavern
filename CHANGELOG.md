...
- **Аудит плавности нативного хоста (десктоп): дёрганый скролл и лагающий
  стрим.** Замеры (release, референс-ПК): холодный `open` документа ~95 мс,
  тёплый produce ~29 мс (`poll` 9 + `resolve` 15 мс — `mark_all_dirty`
  перерисовывает весь односкоуповый шелл; точечные dirty-скоупы — отдельный
  срез после перепланировки шелла). Три фикса горячего пути:

  - **Стрим: один produce на бёрст, ~30 produce/с.** Хост качал по одному
    событию за кадр — каждый токен запускал полную пересборку (29 мс) на
    60 Гц каденса. `ChatSession::pump_stream` выкачивает всю очередь за раз
    (dirty один раз), а гейт 33 мс в `frame` удерживает продюс в пределах
    30/с (AGENTS §24); терминальный кадр поднимает гейт немедленно.
  - **Скролл: гидратация изображений ушла из ленд-критического пути.**
    `refresh_visible_assets` (до 32 wire-фечей) выполнялся внутри produce и
    удлинял каждый ленд; теперь — после present и только вне живого жеста.
    Возврат `bool` («гидратировано что-то новое») поднимает один redraw,
    поэтому свежие картинки реально появляются, а не ждут следующего
    касания (раньше после ленда не было ни produce, ни redraw — плейсхолдеры
    висели до первого клика).
  - **Диагноз зафиксирован в коде:** «обрезание» при скролле — это filler
    полоса blit-шейдера (`BLIT_WGSL`: фрагменты, чей источник выходит за
    полосу, заливаются плоским цветом) на ведущей кромке, до ack-ленда;
    периодический ленд-стойл = тёплый produce.
  - **Bleed-смягчение: drift-кэп ≤ 96px.** Кэп дрейфа frozen-raster хоста
    ограничен (`ack_cap_for_band` = min(пол-Band, 96px)) — полоса-filler на
    ведущей кромке больше не растёт до ~300px мид-флинга, а остаётся
    кромкой экрана; каденс re-produce на wheel-скоростях не меняется.
    Android не тронут (его fast path композитит строки). Полный фикс
    (overscan-запекание) — отдельный срез по шеллу: упирается в однорастровый
    present и сломанный `z-index` Blitz (hoist-баг), дизайн-кандидаты — в
    `docs/desktop/neocompositor-desktop-host.md`.
  - **Клоны view-model ушли из горячего пути.** Установленные шелл/чат
    раздаются рендеру как `Rc` — кадр обходится одним data-клоном чата при
    install вместо трёх полных клонов на кадр; нажатие клавиши и
    pointer-события читают поля драфта напрямую из состояния
    (`route_state()`), а не через сборку `shell_view()` (на символ было две
    сборки view-model, стала одна).
  - **Браузер персонажей: кэш карточек + постраничный load-more.** Фильтр и
    сортировка каталога собираются один раз на изменение (ревизия/поиск/
    сортировка) и раздаются как `Rc`, а не пересобираются и не клонируются
    на каждый produce; сетка раскладывает страницу из 50 карточек
    (React `useCharacters(limit: 50)`), кнопку `Load more` открывает
    следующую — DOM больше не растёт со всем каталогом.

- **Ядро: справедливость писателя и ограниченный Drop (волна 3).**
  `generation.tool.result` исполнялся инлайн внутри emit-дрейна ЧУЖОГО
  стрима — активный стрим стоял на всю длительность provider-turn (до
  60 с): возобновляемый turn теперь уходит в очередь писателя и стартует
  после завершения активного экзекьютора (порядок per-run сохранён;
  обычный сценарий «стрим ждёт tool» не задет — там дрейн не активен).
  `Drop for Kernel` ждал ответа писателя бесконечно — застрявший в
  provider-turn адаптер вешал весь процесс на выходе: shutdown теперь с
  грейсом 5 с и detach застрявшего писателя (документированная утечка
  потока при зависшем адаптере). Тесты: стрим-A не останавливается во
  время tool-result чужого рана; Drop с зависшим провайдером возвращается
  за грейс.

- **Хвост корректности P1 (волна 2).** Семь правок по итогам построчного
  аудита нативного UI:

  - **chats_hit из layout-токенов.** Хиты домашней панели выведены из
    общего `chats_tab::chats_layout` (единый источник с рендером, высоты
    строк пиннятся на `<li>`); строки без `character_label` стали короче на
    мета-строку (59px против 76px) — раньше хит считал все строки по 76px и
    уезжал с третьей строки смешанного списка.
  - **Честный «Context N%».** Триггер контекст-панели показывает реальный
    `usage_percent` (хелпер `context_meter_label`, общий для blueprint-
    ветки и legacy RSX) вместо захардкоженного «Context 4%»; клэмп
    `modal_geometry` не уводит диалоги выше вьюпорта в минус.
  - **Скрипт-чувствительная оценка высоты строк.** `estimate_height`
    считал по UTF-8-байтам (кириллица ×2, эмодзи ×4) — теперь через
    `estimate_tokens`, латиница калибрована 1:1; окно гидрации и
    композиторская база (A3) остаются одним источником.
  - **Карточка деталей переживает скролл владельца.** `details_row`
    резолвится из полного списка сообщений, а не из видимого окна.
  - **JNI: ретраи с синтетическим transport-ключом больше не бьются в
    ядро** — `last_run_id` пропускает ключи `s<pointer>` и берёт реальный
    run id из событий; хелперы стрим-ключей переехали в тестируемый
    `wire.rs`.
  - **Android-сборка починена после среза кэша карточек:** кэш и
    `ProductShellView.characters` на `Arc` (Send для JNI `ROUTE`-Mutex),
    `shell_without_avatars` через `Arc::make_mut`, `produce_and_raster`
    получен обязательный `AssetStore`. Android check зелёный.
  - Зафиксировано ложное срабатывание аудита: «stale `<` → `<=`» в m0-d2
    менять нельзя — события живого роута несут один `generation`, `<=`
    отбросил бы стрим после первого кадра (см. host-doc).

- **Нативный скролл панелей + React-паритет редактора персонажа.** У
  Blitz-краски нет overflow-скролла: хост роутит wheel над панелью в
  `ChatSession::scroll_panel_by` (клэмп по высоте отрендеренного контента
  через `HitRects::subtree_bottom`), тело редактора применяет смещение
  отрицательным margin, узел ре-креируется сменой `key` (Blitz не
  перекрашивает сменившийся inline-стиль на переиспользованном узле).
  Смещение сбрасывается при смене панели/вкладки/персонажа. Режим карточки
  выровнен по React `selectCharacter`: выбор персонажа (в т.ч. повторный
  тап) всегда открывает read-only viewer, в редактор ведёт кнопка-карандаш
  (`ToggleCharacterEditorMode`), дефолт панели — `view`, а не `edit`.
  Кнопки редактора (back/favorite/export/duplicate/delete) несут настоящие
  `data-action` и резолвятся через hit-rects (`character_custom_action`) —
  жёсткие пиксельные полосы `editor_hit` заменены чистым Absorb (паттерн
  M1). Inline-оверрайды message-header/action-bar сняты симметрично в
  blueprint-ветке и legacy RSX: раскладку даёт packed-класс (React golden
  CSS: flex row, wrap, gap 12px / wrap + gap 4px) — паритет blueprint↔legacy
  восстановлен.
...
- **Персона: редактор (personas.update).** Edit-таб: имя и описание с фокусом
  ...
  `persona_meta_update_over_product_wire`.

- **Лорбук: редактор книги (lorebooks.update).** Book-таб: имя и описание
  с фокусом клавиатуры, кнопка Save; на провод идут только изменённые поля,
  пустое имя хранит старое, no-op — без wire-вызова. Тест
  `lorebook_meta_update_over_product_wire`.
...
- **Иконки навигационного рейла были прижаты к правому краю вместо центра.**
  ...
  вместо 9.5 (позиция flex-end) — все глифы рейла визуально уезжали вправо на
  ~8-10px при корректной раскладке кнопки-пилюли. Центрирование контейнеров с
  процентной шириной работает корректно, поэтому обход — без flex-центрирования
  вообще: `icon_fill_centered` делает спан иконки полным размером кнопки со
  статическим `padding: (40-size)/2` и `box-sizing:border-box`; то же для
  menu-toggle. Заодно убран никогда не отображавшийся `Sidebar_railLabel` из
  кнопок рейла (aria-label остаётся на кнопке). Goldens обновлены (иконки
  сдвинуты к паритету React), проверено на 283×945 и 1920×1080: все кластеры
  глифов 22..38 при кнопках 10..50; cargo-тесты, goldens 0.0000% (4 размера),
  packaged-check 6/6 + no-clip guard.

- **No-clip guard: панель персонажей больше не может рисоваться за краем
  узкого окна незамеченной.** Панель — fixed-width лист (380px, clamp
  260..720); при viewport ≤ 600 компакт-ветка раскрывает её на всю ширину
  окна минус рейл (паритет React `@media (max-width: 600px)`), и на 283×945
  контент остаётся в границах окна. Чтобы класс дефекта «адаптивность
  сломалась и контент обрезался» больше не доходил до пользователя, в
  `pnpm blueprint:packaged-check` добавлен этап 7 (no-clip guard): хост
  рендерится на 283×945, 360×640 и 900×220, и пиксельная проверка asserts,
  что ни один светлый пиксель контента не лежит в пределах 2px от правого/
  нижнего края (тёмные full-bleed фоны игнорируются; колонка рейла у нижнего
  края исключена — нижняя иконка рейла прибита к инсету и подрезается только
  на суб-минимальной высоте, в продукте min-height 520). Размерное соглашение
  и контракт compact-переключения задокументированы в
  `docs/desktop/rust-ui-style-port.md`.
