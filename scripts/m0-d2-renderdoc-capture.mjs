#!/usr/bin/env node
/**
 * RenderDoc v1.45 Android capture for M0-D2 at generation 120.
 * Not D2 PASS. Does not rewrite D1a evidence. Does not flip the probe capture bit.
 *
...
const ACTIVITY = 'com.neotavern.mobile.M0D2Activity';
...
function captureFilenamesD2(stamp) {
  const base = `${stamp}-d2`;
  return {
    stamp,
    rdc: `${base}.rdc`,
...
function launchProbe(adbBin, serial, captureFrame) {
  ...
  return adb(adbBin, serial, [
    ...
    `${PACKAGE}/${ACTIVITY}`,
    '--es',
    `${PACKAGE}.M0_D2_FRAMES`,
    '1000',
    '--es',
    `${PACKAGE}.M0_D2_CAPTURE_FRAME`,
    String(captureFrame),
  ]);
}
...
function waitForProbe(adbBin, serial, mode) {
  ...
  while (Date.now() < deadline) {
    ...
    const gpuRan = /m0-d2 gpu_ran=true/.test(log);
  ...
  return {
    ...
    gpuRan: /m0-d2 gpu_ran=true/.test(log),
...
function listCaptureCandidates(adbBin, serial) {
  ...
  for (const line of (viaRunAs.stdout || '').split(/\r?\n/u)) {
    ...
    if (name.endsWith('.rdc') && name.includes('m0-d2')) {
...
function main() {
  ...
  const counters = parseD2LogLine(waited.log);
  ...
  if (mode === 'control') {
    ...
    printJson({
      ...
      note: 'control 1000-frame D2; RenderDoc extra=-1; not a D2 PASS',
    });
  ...
  if (pulled.ok) {
    ...
    if (converted.ok) {
      ...
      completeness = classifyD2Dump(dump);
      const labels = extractDebugLabels(readFileSync(xmlPath, 'utf8')).filter((name) =>
        name.startsWith('m0-d2-'),
      );
      order = checkD2PassOrder(labels);
  ...
  const manifest = buildEvidenceManifest({
    ...
    unblock: readable
      ? 'review Event Browser; completeness check is not D2 PASS'
      : 'RenderDoc capture is not a readable Vulkan generation-120 tree yet; D2 stays STARTED',
  });
  writeFileSync(
    devicePath,
    `${JSON.stringify(
