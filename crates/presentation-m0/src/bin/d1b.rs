fn main() {
    match neotavern_presentation_m0::gpu::run_dynamic_d1b(1000) {
        Ok(report) => {
            ...
            println!("moving_sample_blits={}", report.moving_sample_blits);
            println!("pass_compiles={}", report.pass_compiles);
            println!("vello_rebuilds={}", report.vello_rebuilds);
            println!("layout_rebuilds={}", report.layout_rebuilds);
            println!("ui_rebuilds={}", report.ui_rebuilds);
            println!("render_thread_polls={}", report.render_thread_polls);
            println!("capture_only_polls={}", report.capture_only_polls);
            println!("sampled_generation={}", report.sampled_generation);
            println!(
                "damage={}x{}+{}x{}",
                report.damage_x, report.damage_y, report.damage_w, report.damage_h
            );
            println!("capture_timeline={}", report.capture_timeline);
            ...
            println!("{}", report.to_d1b_log_line());
        ...
        Err(err) => {
            eprintln!("M0-D1b GPU probe failed to start: {err}");
            std::process::exit(2);
        }
    }
}
