...
/// Enable naga shader bounds checks for the next [`Renderer::new`].
///
/// Upstream Vello 0.9 compiles compute WGSL with
/// [`wgpu::ShaderRuntimeChecks::unchecked`]. Diagnostic A/B captures may turn
/// checks on; that is not a WGSL patch and not a production default.
#[cfg(feature = "wgpu")]
pub fn set_debug_shader_bounds_checks(enabled: bool) {
    wgpu_engine::set_debug_shader_bounds_checks(enabled);
}

/// Whether the next [`Renderer::new`] will compile shaders with bounds checks.
#[cfg(feature = "wgpu")]
#[must_use]
pub fn debug_shader_bounds_checks() -> bool {
    wgpu_engine::debug_shader_bounds_checks()
}
