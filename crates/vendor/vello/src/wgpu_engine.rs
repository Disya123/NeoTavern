...
static DEBUG_SHADER_BOUNDS_CHECKS: AtomicBool = AtomicBool::new(false);

pub(crate) fn set_debug_shader_bounds_checks(enabled: bool) {
    DEBUG_SHADER_BOUNDS_CHECKS.store(enabled, Ordering::SeqCst);
}

pub(crate) fn debug_shader_bounds_checks() -> bool {
    DEBUG_SHADER_BOUNDS_CHECKS.load(Ordering::SeqCst)
}

fn shader_runtime_checks() -> wgpu::ShaderRuntimeChecks {
    if debug_shader_bounds_checks() {
...
impl WgpuEngine {
    ...
    fn create_compute_pipeline(
        ...
        // Default matches upstream `unchecked()`. Diagnostic Android builds may
        // enable naga bounds checks via [`crate::set_debug_shader_bounds_checks`]
        // for an A/B capture; do not treat that as a WGSL patch.
