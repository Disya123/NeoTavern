...
impl BaseDocument {
    ...
    /// NeoTavern bounded patch (producer seam): live + detached node count,
    /// read-only. With the arena GC disabled (see `mutator.rs`), detached
    /// subtrees accumulate here; hosts use this to decide when to cold
