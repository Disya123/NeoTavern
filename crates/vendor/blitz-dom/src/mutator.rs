...
impl DocumentMutator<'_> {
    ...
    pub fn remove_node_if_unparented(&mut self, node_id: usize) {
        // NeoTavern bounded patch (producer seam): disabled GC. Upstream
        // drops the node from the arena here, but dioxus-native-dom
        // 0.8.0-alpha.1 keeps every ElementId → node mapping alive across
        // the drop, so later mutations (push_root, path walks, inserts)
        // index arena-dead ids and panic with `invalid key` — reproducibly
        // when the virtualized chat window shifts rows (id recycling).
        // Detached nodes now stay in the arena instead; see
        // NEOTAVERN_PATCH.md for the leak accounting.
        let _ = node_id;
    }

    /// Remove all of the children from old_parent_id and append them to new_parent_id
    pub fn append_children(&mut self, parent_id: usize, child_ids: &[usize]) {        self.add_children_to_parent(parent_id, child_ids, &|parent, child_ids| {
    ...
    fn add_children_to_parent(
        ...
        for child_id in child_ids.iter().copied() {
            ...
            if let Some(old_parent_id) = old_parent_id {
                // NeoTavern bounded patch (producer seam): dioxus-native-dom's
                // ElementId recycling can drop a detached node from the arena
                // (`remove_node_if_unparented`) while its descendants keep
                // `parent` pointing at the now-dead id. Re-parenting such a
                // child must not index the dead old parent — the cleanup
                // below only refreshes damage bookkeeping, which a removed
                // node no longer needs.
                if self.doc.get_node(old_parent_id).is_none() {
                    continue;
                }
