...
| `memories.update`                  | transactional         | `PATCH /api/v2/memories/:id` (kernel mode: wire op `memories.update`, Этап 4 slice 3; no `characterId: null` on the wire — un-scoping is CAPABILITY_UNAVAILABLE; switching scope to `character` requires `characterId` — same validation as `memories.create`)                                                                                                                                                                                                                                 |
