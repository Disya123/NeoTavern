...
- **Не центрируйте иконку flex'ом внутри фиксированной кнопки.** Taffy в этом
  Blitz-билде кладёт fixed-size span (21×21) внутри fixed-size flex-контейнера
  (40×40, `justify-content:center`/`margin:auto`) в `loc.x = 18` вместо 9.5 —
  позиция flex-end, иконки визуально уезжают к правому краю рейла. Центрирование
  контейнера с `width:100%` (item→button) работает корректно. Рабочий паттерн:
  спан иконки = полный размер кнопки + статический `padding: (box-size)/2` +
  `box-sizing:border-box` (`icon_fill_centered` в `product_shell.rs`) — никакой
  зависимости от flex-центрирования. Метод локализации: временные trace-точки
  в vendor `render.rs` (transform vs taffy loc по цепочке paint-предков).
