...
- **Драйв кадров**: `about_to_wait` крутит `request_redraw` +
  ...
  чередовались 7/14 мс и скролл джаддерил. Каждый кадр сэмплирует анимацию
  (visual offset) и презентует blit-сдвиг запечённого растра; продьюсы —
  только на лендах по ранвею (см. «Оверскан-ранвей»). Простой окна
  по-прежнему 0% CPU (`ControlFlow::Wait`).
...
`--dom-dump`/`--snapshot` — путь доживает до доводочного кадра.
...
- **Blit-маппинг**: экран мапится в среднее окно растра
  ...
  к высоте растра; `scroll[4].xy` — окно источника, за которым работает
  старая ветка заливки обоями. Юниформ расширен до 5 строк (80 байт) у
  обоих хостов; Android пишет flat-маппинг и старый банд — его поведение
  не менялось.
- **Направленные капы из ранвея**: `ScrollAckLoop::set_runway_caps(older,
  newer)` — кап вверх = закрашенный запас над полосой, вниз = под ней
  (клипнутые боксом extents канвы едут в поле зрения
  `chat_canvas_top_css`/`chat_canvas_bottom_css`). На нижнем пине запас
  вниз ~0, но туда нельзя скроллить — кап вниз ничего не режет.
