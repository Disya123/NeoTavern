...
- **Панельная заливка прозрачных сэмплов**: всё, что не покрывают обои,
  ...
  чёрная полоса. Честная семантика ранвея: DOM-покраска не рисует строки
  выше css 0 (вендорский клип обхода = окно), поэтому обнажаемый край —
  плоская заливка, а реальные строки доезжают лендом; направленные капы
  задают каденс пересадок, а не пиксельную гарантию.
