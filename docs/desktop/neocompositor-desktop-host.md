...
- старт (офсет 0): новые строки прижаты к низу — lastBottom=819 при
  вьюпорте до 827 (хвост шага), окно = последние строки;
- `--wheel 37` → `--wheel 137`: те же строки сдвинулись ровно на +100px,
  без смены набора строк — непрерывность;
...
- контракт-тест `chat_scroll_window_is_continuous_and_pins_both_edges`
  (live_wire) закрепляет все три инварианта;
