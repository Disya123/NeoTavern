...
- **Направленные капы из ранвея**: `ScrollAckLoop::set_runway_caps(older,
  ...
  по окончании жеста остаток садится через ack-эпсилон. Сторож в
  `about_to_wait`: если анимаций нет, а дрейф > ack-эпсилон — немедленный
  `land_now` + перерисовка. Ленд живет только на презентующих кадрах
  (кадр продьюса возвращается до фазы present), поэтому жест, чей ease
  пережил последний present, мог оставить петлю припаркованной на
  СДВИНУТОМ растре навсегда — контент сидел смещенным за шапкой/пилюлей
  до следующего ввода (пустоты «в покое» у верхней кромки банды). В покое
  сдвинутый экран недопустим — остаток гасится в той же итерации цикла.
...
- **Chrome-split: хром красится в отдельные сцены и СВОИ текстуры.** Один
  обход дерева (`ProductVelloSession::paint_split`, m0-d2) выдаёт ТРИ
  vello-сцены: контент + хедер + пилюля. Маршрутизация — в вендорном
  blitz-paint: у корня `chrome-block`-поддерева painter вызывает
  ...
  внутри `composer-sticky` (её собственный location = 0). `ChromeRouterSink`
  (m0-d2) делегирует каждый вызов сцены по маршруту; producer-стрим пишется
  только с Main-маршрута.
  **Трёхтекстурный контракт (переписан 2026-09).** Продьюс зовёт
  `PresentSurface::enable_chrome_split(header_h, composer_h)` +
  `render_split(...)`: контент рендерится в оверскан-растр контента
  (`+overscan` — экран сэмплирует среднее окно), хедер и пилюля — в СОБСТВЕННЫЕ
  текстуры, размером ровно с их экранные зоны (хедер — 1:1 с doc y 0; пилюля —
  сдвиг вверх на `band_bottom`). В контентном растре НЕТ полос хрома — ни
  офсетов-полос, ни координатной расшивки в шейдере. Первая версия сплита
  упаковывала все три сцены в одну текстуру магическими офсетами и
  расшивала их в шейдере геометрией (`chrome_y = doc_y ± top`) — эта
  координатная супа породила весь класс багов (призрак композера, коллизии
  src-окна, неподвижная копия); контрактом «три текстуры + явный композит»
  класс закрыт конструктивно.
- **Композит хрома в шейдере**: экранные зоны хрома (вне банды) сэмплируют
  свои текстуры по IDENTITY screen-uv и накладывают их поверх сэмпла
  контента как source-over (`rgb = chrome.rgb + rgb·(1−chrome.a)`) —
  полупрозрачный хром честно смешивается со строками позади, ровно как
  инлайн-покраска до сплита. Простое сложение (без альфы) пересвечивало хром.
  Банд свободен от хрома по построению: хром живёт в отдельных текстурах,
  дрейф физически не может занести его в контент.
  **Сдвиг зон хрома**: сэмпл контента ПОД хромом сдвигается тем же дрейфом,
  что и банда (сдвиг действует на всю чат-колонку, не только на банду).
  Раньше зоны читали контент по несдвинутому doc_y — под полупрозрачной
  шапкой и пилюлей стояла НЕПОДВИЖНАЯ копия сообщений, пока банда ехала.
  Вне окна источника (сэмпл мимо запечённого канваса — например, паддинг
  у пина) сэмпл откатывается к несдвинутому doc_y: зоны хрома показывают
  фон, запечённый за ними, а НЕ плоский filler — filler-ветка действует
  только внутри банды, иначе она затирает композит хрома (хедер темнел до
  цвета панели в покое).
  Бокс вьюпорта симметричный (`top: −CHAT_OVERSCAN_CSS; bottom: 0`, без
  `overflow:hidden`) в обоих рендерах: ранвей сверху полный, снизу его
  держит окно выбора сессии; кламп канваса в продьюсе ограничен краями
  растра (`[−CHAT_OVERSCAN_CSS, css_h + CHAT_OVERSCAN_CSS]`), а направленные
  капы ack-петли зажаты `ACK_CAP_MAX_CSS` (96px), чтобы ведущий край filler
  не разрастался.
...
- **Строки в ранвее (стадия 3): корневой клип покраски расширен на оверскан.**
  ...
  окна выбора.
- **Без контейнерного клипа вьюпорта.** `overflow:hidden` на
  chat-viewport (оба рендера: legacy lib.rs и blueprint scene_chat.rs)
  сужал клип потомков до бокса контейнера — scrollport-сужение вендорского
  обхода отсекало запасные ряды, разложенные выше/ниже бокса, а они нужны
  блиту на свипе. Клип снят: ряды ограничивает окно выбора, покраску —
  расширенный корневой клип, а окно источника блита — кламп канваса в
  продьюсе (`canvas_top ≥ −(CHAT_OVERSCAN_CSS − header_h)` держит сэмпл
  вне полос хрома); утечка рядов в полосы перекрашивается сценами хрома
  (они аппендятся последними). Android без изменений: односценовый
  `paint()` режет корневым клипом, как раньше.
