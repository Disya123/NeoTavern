...
- **Blit-маппинг**: экран мапится в среднее окно растра
  ...
  их из кэша продьюса `chat_canvas_css`). Оно НЕ равно всему растру: под
...
- **Направленные капы из ранвея**: `ScrollAckLoop::set_runway_caps(older,
  ...
  ПОЛОЖИТЕЛЬНЫЙ дрейф (скролл вниз) обнажает НИЖНИЙ край полосы и
  ограничен запасом ПОД полосой (`cap_newer`), отрицательный — верхним
  (`cap_older`). Раньше направления были перепутаны, и вниз-скролл
  проверялся против верхнего ранвея, пока нижний край сэмплировал композер.
  Кэп берётся из клипнутых боксом extents канвы
  (`chat_canvas_top_css`/`chat_canvas_bottom_css`). На нижнем пине запас
  вниз ~0 (под полосой рядов нет — там композер): кап садится на пол
